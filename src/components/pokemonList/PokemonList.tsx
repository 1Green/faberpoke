...
export const PokemonList: FunctionComponent<PokemonListProps> = ({
  ...
  if (isLoading) return <p className={styles.informationText}>Loading...</p>;
  if (pokemons === undefined || pokemons.length === 0) {
    return <p className={styles.informationText}>No Pokemon found.</p>;
  }
  return (
    <div className={styles.pokemonListWrapper}>
      {pokemons.map((pokemon, index) => (
        <div
          ...
          onClick={() => onShowPokemon(pokemon.url)}
        >
          <img
            alt="pokemon"
